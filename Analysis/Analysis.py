    ...
    for doc in cursor:
        ...
        if str(doc["_id"]) in duplicates:
            ...
            if len(doc["Related Listings"].keys()) > 0:
                ...
                if randomKey is not None:
                    for listing in doc["Related Listings"][randomKey]:
                        if masterList.count(listing) == numKeys and numKeys >= 3:      # all attributes of the listing is the same
